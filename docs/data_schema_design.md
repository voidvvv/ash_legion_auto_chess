...
> **版本**：V1.6（机会制常量：`DEFEAT_LIMIT_PER_ROUND` 新增 / `MERCY_*` 两常量删除；败箱"每轮 ≤2"改由机会制推导）  
...
| 经济 | 起始金币 / 刷新 / 买经验 / 宝箱公式 / 经验书 / 败箱 / 战败机会上限 | 10 / 2 / 4 / 3+floor(轮/3)、**封顶 12**、Boss×2（仅金币）/ **4+floor(轮/5)**（Boss 不加倍）/ 金币=胜箱×50% 向下取整、经验书同值、无装备槽、**第 1/2 败各 1 个（第 3 败终局无箱，“每轮 ≤2”由机会制推导不独立设参）**、零棋子不给但照常耗机会 / **`DEFEAT_LIMIT_PER_ROUND = 3`**（第 3 败 RUN_END(DEFEATED)、新轮清零；怜悯常量已删）——以上均待调（2026-08-23） |
| 商店 | 槽位数 / 费阶概率锚点 | 5 / GDD §3.4 锚点表 |
| 棋盘 | 尺寸 / 敌我分区 / 备战席 | 6×7 / 敌0~2·缓冲3·我4~6 / 9 格 |
| 等级 | 人口表 / 经验需求 | GDD §3.5 表（2026-08-23 压平：4/8/12/20/28/36，总需求 108，待调） |
| DOT | 心跳间隔 | 1 s |
...
| 2026-08-23 | 经济常量联动（手验修订） | 宝箱金币封顶 10→**12**；经验书 +4 固定→**4+floor(轮/5)**（Boss 不加倍）；棋手经验表压平 **4/8/12/20/28/36**；新增败箱常量（金币 = 胜箱 ×50% 向下取整 / 每轮 ≤2【待确认】）——规则见 GDD §2.2/§3.2/§3.5，全部待调 |
| 2026-08-23 | 机会制常量（用户裁决） | **删** `MERCY_START_LOSS` / `MERCY_CAP_PER_ROUND`（怜悯整体拆除，GDD §3.2 行同步删除）；**增** `DEFEAT_LIMIT_PER_ROUND = 3`（每轮战败机会上限，工作值待调）；败箱"每轮 ≤2"撤销独立常量、由机会制推导（第 1/2 败各 1 箱，第 3 败终局无箱）——规则见 GDD §2.2/§3.2，代码锚点见 architecture §5.4 |
