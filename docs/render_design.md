...
> **版本**：V1.6（**§5.6 改写：开战倒计时横幅退役 → 开战转场「清场入阵」**——转场动作表 / 战毕反向转场 / 镜头回正规格，§九 ②③ 阶段可见性同步修订——`battle_design.md` V1.8 §二开战转场的渲染侧落点，2026-09-09 裁决；前一版 V1.5：攻击蓄力条（保留）+ 开战倒计时横幅（已随本次修订退役）；V1.4：RESULT 弹窗三分岔 + 攻击/受击动作反馈挂链 `attack_feedback_design.md`）  
...
③ 备战席格子、出售区
④ 棋子（UnitView：插值位置 + 动画 + 血条/能量条/攻击蓄力条/星级底光）
⑤ 弹道（ProjectileView）
⑥ 特效层（FxLayer）
⑦ 飘字层（FloatingText）
⑧ 拖拽 ghost（半透明，跟随指针）
...
### 5.6 攻击蓄力条与开战转场「清场入阵」（2026-09-09 改写：倒计时横幅退役；battle_design V1.8 §二/§5.1 的渲染侧落点）

**攻击蓄力条（UnitView 第四条微条，纯表现零逻辑）**：
- **数据源：逐帧轮询，不发新事件**——与 §5.4 状态图标同走"轮询"路线：`ratio = min(1, attackTimer / attackInterval)`（attackTimer 恒累计可超 interval，显示**钳制满格**——被眩晕 / 未进射程时满格悬停，"蓄满待发"视觉语言）
- **绘制**：`UnitView.drawBars()`（`render/board/UnitView.java:141`）在血条（cy+17，2px）与能量条（cy+19，1px）之下新增第四条：**y = cy+21、高 1px、宽 24px（与上两条同宽同 x 基准）**，暗色底槽 + 白色前景（与红 / 绿 / 黄 / 金四色全部区分）；样式工作值待调，并入 §十一"血条/能量条最终样式"待定项
- **语义对照**（battle §5.1）：蓄满即出手 → 出手取模归零即**清空重蓄**；移动中照常蓄力（恒累计）；开战转场期间恒为空条
- **边界**：不占动画状态位（非 FSM 态、非叠加特效层，与血 / 能量条同为静态微条）；不产生任何 CombatEvent；近战 / 远程 / 敌我同规格

**开战转场「清场入阵」（2026-09-09，取代原开战倒计时横幅——横幅 `render/ui/BattleIntroBanner.java` 退役删除，`BATTLE_INTRO_GO_BEAT_SECONDS` 废弃，「3 → 2 → 1 → 开战！」节拍作废）**：
- **依据与时序**：分镜 `docs/diagrams/battle_intro_slide_transition.md`（配 .html 三帧分镜 + 动效演示）；时长 **0.6s（0.4~0.8 待调）**——`GameBalance` 转场常量（暂名 `BATTLE_INTRO_TRANSITION_SECONDS`，取代 `BATTLE_INTRO_COUNTDOWN_SECONDS`，终名由实施计划定）；BATTLE 进入即播、归零即解冻进主循环，每场战斗播一次（重试重新播）。逻辑冻结语义在 battle §二（`introRemaining` 门控复用），转场驱动只是其表现
- **驱动**：转场驱动器逐帧消费 `introRemaining` 归一化进度（0→1）驱动动作；UI 域 `uiStage` actor 位移/淡入出 + 棋盘域 `worldCamera` zoom 插值——纯表现层，零逻辑改动、零 CombatEvent、零 RNG
- **动作表**（0~0.6s 内完成；区域编号沿用 §九坐标表）：

| 元素（域） | 动作 |
|---|---|
| 商店栏 ⑧（UI，实机底部） | **下滑离场**（屏幕下缘；反向**上滑回位**。Q6 勘误：实机渲染在底部 0..64 条带——§九 ⑧ 表值与代码漂移属既有问题） |
| 战斗 HUD（UI，实机顶部） | 自屏幕上缘外**下落就位**（反向**上升退场**；BATTLE 期常驻顶部，**不接管任何槽位**——「交接同一槽位」表述废弃，两屏缘各自进出，观感 = 经营层/战斗层整层交换；决策 #4 静态语义不变） |
| 开战按钮 ⑥（UI） | 左移淡出 |
| 出售区 ⑦（棋盘域）+ 布阵提示 | 淡出 |
| 备战席 ②（棋盘域）+ 背包 ③（UI） | **左滑离场**（Q1 裁决；§九 ②③ 阶段可见性由「全程、战斗期置灰」修订为「仅 SHOPPING」） |
| 羁绊面板 ⑤（UI） | 原地置暗保留（羁绊是战斗相关信息，清场不清它） |
| 通知 ⑨ / 顶栏 | 不动 |
| 棋盘 ④（棋盘域） | **镜头轻微回正 zoom 1.06→1.0**（Q4 保留——镜头运动仅此一处，不随单位移动）；敌阵侦察虚影实体化 = 敌军亮相一拍 |

- **反向转场（战毕回备战 RESULT→SHOPPING，Q3 已定）**：同一套动作反向播——商店栏上滑回位、战斗 HUD 上升退场、备战席/背包滑回、开战按钮/出售区淡入、镜头复位回 zoom 1.06；羁绊面板/通知/顶栏不动（方向为 Q6 勘误后口径）
- **交互**：转场期**输入禁用**（棋盘与 UI 一并禁用——unproject 命中无歧义；战斗 HUD 的变速/投降在转场结束后照常可用）；×2 快进同步加速转场（battle §二快进交互）
- **边界**：不新增弹窗、不做单位出场演出；镜头运动仅此一处回正插值（原「MVP 无镜头」边界据此修订——镜头仍不随单位移动、不做事后运镜）
...
│   ├── UnitView.java           #   单位视图：动画FSM/插值/血条/能量条/攻击蓄力条/星级底光（id 索引）
...
> **逐 key 规格清单**：全部素材的 key / 尺寸 / 类型 / 消费点 / 现状注意事项（含 Boss 尺寸压缩、panel 非九宫格两处现状差异）见 `art_asset_spec.md`（2026-08-22 起，随代码现状维护）。
...
| ② | 备战席 3×3 | (20, 48, 108, 120) | **棋盘域** | 仅 SHOPPING（随开战转场左滑离场——2026-09-09 修订，原「全程（战斗期置灰）」） |
| ③ | 装备背包 3×2 | (20, 172, 108, 72) | UI | 仅 SHOPPING（随开战转场左滑离场——2026-09-09 修订，原「全程（战斗期置灰）」）；原表值 (20,140,108,100) 与 ② 压叠 28px，feedback01 修正 |
...
| ⑧ | 商店栏 | (0, 296, 640, 64) | UI | 仅 SHOPPING（随开战转场下滑离场；Q6 勘误：实机渲染在**底部** 0..64 条带——`BattleScreen.java:226` 装配无 setPosition、`ShopBar.java:29` 定位注释未落地，本表坐标与代码漂移属既有问题，修正另行裁决） |
| — | 战斗 HUD | (0, 296, 640, 64) | UI | 仅 BATTLE（随开战转场自屏幕上缘外下落就位——实机常驻顶部（`BattleHud.java:43,46` 子件 VIRTUAL_H−46），与 ⑧ 分处两屏缘；变速 ×1/×2、60s 计时条、投降） |
...
- [ ] 血条 / 能量条 / 攻击蓄力条（§5.6，2026-09-02 新增）/ 星级底光的最终样式（棋子下方微型条）
- [ ] 开战转场工作值（时长 0.6s 待调、区间 0.4~0.8；滑出/淡出位移与缓动、羁绊面板置暗透明度、镜头回正在 0.6s 内的时长配比——机制已定 §5.6，2026-09-09）
...
| 2026-09-02 | 战斗节奏表现层（V1.5） | **§5.6 双件**：攻击蓄力条（`UnitView.drawBars()` 第四条微条：y=cy+21 高 1px 宽 24px、轮询 `attackTimer/attackInterval` 钳制满格、出手清空重蓄、移动中照常蓄力）+ 开战倒计时横幅（UI 域 `ResultBanner` 同通路，中央大字倒数、不拦截输入）——纯表现零逻辑，依据 `battle_design.md` V1.7 §二开战铺垫 / §5.1 |
| 2026-09-09 | 开战转场「清场入阵」（V1.6，§5.6 改写） | **倒计时横幅退役（`BattleIntroBanner` 删除、`BATTLE_INTRO_GO_BEAT_SECONDS` 废弃）→ 清场转场**：动作表（⑧ 上滑 / 战斗 HUD 自顶部下落接管同槽 (0,296,640,64) / ⑥ 左移淡出 / ⑦+布阵提示淡出 / ②③ 左滑离场 / ⑤ 置暗保留 / 镜头回正 zoom 1.06→1.0 + 敌阵虚影实体化）+ 战毕回备战反向同款转场（Q3）；驱动仍走 `introRemaining` 门控、时长换转场常量（暂名 `BATTLE_INTRO_TRANSITION_SECONDS`）；§九 ②③ 可见性修订为仅 SHOPPING（原「全程、战斗期置灰」）；转场期输入禁用；镜头上摇变体否决（Q5）——依据 `battle_design.md` V1.8 §二 / 分镜 `battle_intro_slide_transition.md` |
| 2026-09-09 | 开战转场 Q6 勘误（§5.6/§九 同步修正，维持 V1.6 不升版） | planner 核实：**⑧ 实机在底部**（`BattleScreen.java:226` 装配无 setPosition = 底部 0..64 条带；`ShopBar.java:29` 定位注释未落地）、**HUD 实机在顶部**（`BattleHud.java:43,46` 子件 VIRTUAL_H−46），§九 ⑧ 表值与代码漂移属既有问题（坐标修正另行裁决，仅登记）。§5.6 动作表改为：⑧ **下滑离场 / 上滑回位**、HUD **自屏幕上缘外下落就位 / 上升退场**（常驻顶部）；「交接同一槽位」表述全线废弃（两屏缘各自进出，观感 = 经营层/战斗层整层交换）；§九 战斗 HUD 行同步改写、⑧ 行加漂移登记 |
